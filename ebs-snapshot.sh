...
#
# Edited by Alexander Fedorko (alx69, alx@alx.kiev.ua) at 2020-11-27
#
...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		log "New snapshot is $snapshot_id (volume_id is $volume_id)"
	 
		# Add a "CreatedBy:AutomatedBackup" tag to the resulting snapshot.
		# Why? Because we only want to purge snapshots taken by the script later, and not delete snapshots manually taken.
		aws ec2 create-tags --region $region --resource $snapshot_id --tags Key=CreatedBy,Value=AutomatedBackup Key=Name,Value=$volume_id
	done
}
...
volume_list=$(aws ec2 describe-volumes --region eu-central-1 --filters Name=tag:Name,Values=do-prod-* --query Volumes[].VolumeId --output text)
